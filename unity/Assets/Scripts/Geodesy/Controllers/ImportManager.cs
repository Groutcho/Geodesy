...
namespace OpenTerra.Controllers
{
	public class ImportManager
	{
		...
		/// <summary>
		/// Tries to import the specified URI by selecting the exporters
		...
		private Response ExecuteImportCommand(Command command)
		{
			...
			try
			{
				...
				Import(uri);
				return new Response(string.Format("{0} successfully loaded.", uri), ResponseType.Success);
			}
			catch (Exception ex)
			{
