...
namespace OpenTerra.Unity
{
    public class Globe : IGlobe
	{
		...
		public Vector3 Position { get { return Converter.FromCartesian3(datum.Transform.Position); } }
		...
		private void DrawEllipse (Ellipse ellipse, float from, float to, float resolution, Color color)
		{
			...
			{
				Vector3 vfrom = Converter.FromCartesian3(ellipse.Sample (longitude)) * reductionFactor;
				Vector3 vto = Converter.FromCartesian3(ellipse.Sample (longitude + resolution)) * reductionFactor;
		...
		private void DrawAxes ()
		{
			...
			Gizmos.DrawLine (Converter.FromCartesian3(orig), Converter.FromCartesian3(semimaj));
			Gizmos.color = Color.blue;
			Gizmos.DrawLine (Converter.FromCartesian3(orig), Converter.FromCartesian3(semimin));
		}
